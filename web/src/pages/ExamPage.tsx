...
import { DOMAIN_LABELS, formatDuration, getExam, isCorrect, phases } from '../lib/content';
...
export default function ExamPage() {
  ...
  if (stage === 'result' && result) {
    return <ExamResult exam={{ ...exam, questions }} attempt={result} onRetry={start} />;
  }
...
function ExamResult({
  ...
  const nextPhase = phases.find((p) => p.order === (phases.find((p) => p.gateQuiz?.id === exam.id)?.order ?? 0) + 1);
      ...
      >
        ...
        <div className="mt-5 flex flex-wrap justify-center gap-2">
          ...
          {attempt.passed && exam.kind === 'gate' && nextPhase?.ready && (
            <ButtonLink to={`/phase/${nextPhase.id}/notes/${nextPhase.notes[0]?.id ?? ''}`}>
              Sang Phase {nextPhase.order} →
            </ButtonLink>
          )}
          {!attempt.passed && <ButtonLink to="/review">Ôn câu sai</ButtonLink>}
